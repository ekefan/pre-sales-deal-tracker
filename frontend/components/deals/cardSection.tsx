...
export function CardSection({ page }: Props) {
  ...
  useEffect(() => {
    async function getDeals(token: string | undefined, url: string, param: Status | DealFilter) { /* defined an async function */
      try {
        if (!token) {
    ...
    if (page === "home") {
     ...
     const param = dealParam != null ? dealParam :  {
      customer_name: null,
  ...
  return (
    <div className="flex flex-col gap-3  p-3 w-full grow h-auto">
      <section className="flex flex-col w-full h-full gap-3 ">
        {deals.length === 0 ? (<>no deals</>) : (deals.map((deal) => {
